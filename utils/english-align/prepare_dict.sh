...
cat conf/cmu2pinyin | awk '{print $1g;}' | sort -u > $dict_dir/map/cmu || exit 1;
...
if [ ! -f $dict_dir/cedict/cedict_1_0_ts_utf-8_mdbg.txt ]; then
  ...
  gzip -d $dict_dir/cedict/cedict_1_0_ts_utf-8_mdbg.txt.gz
fi
